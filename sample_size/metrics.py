...
class BaseMetric:
    ...
    def generate_p_values(self, true_alt: npt.NDArray[np.bool_], sample_size: int) -> npt.NDArray[np.float_]:
        ...
        p_values[~true_alt] = stats.uniform.rvs(0, 1, total_null, random_state=1)
...
class BooleanMetric(BaseMetric):
    ...
    def _generate_alt_p_values(self, size: int, sample_size: int) -> npt.NDArray[np.float_]:
        ...
        z_alt = stats.norm.rvs(loc=effect_size, size=size, random_state=1)
...
class NumericMetric(BaseMetric):
    ...
    def _generate_alt_p_values(self, size: int, sample_size: int) -> npt.NDArray[np.float_]:
        ...
        t_alt = stats.nct.rvs(nc=nc, df=2 * (sample_size - 1), size=size, random_state=1)
